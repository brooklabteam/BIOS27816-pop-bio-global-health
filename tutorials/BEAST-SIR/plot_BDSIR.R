...
# Load your data
data <- read.table('NorthAm.Nov.BDSIR.log', head = T)
# Set parameters for time
most_recent_sample <- 2009
burnin <- 3000
data <- data[burnin:nrow(data), ]

# Pull estimates of state variables
...
# And plot estimates of R0 as a histogram
hist(data$reproductiveNumberEs, main=bquote(R[0]), xlab="Basic reproductive number")


# Can you use Gwen's script to plot the tree file in R?
# Conversely can you open it in FigTree to look at it?
